...
def log_beta_gen(dp, end_dp=None):
    ...
    yield log_beta
    for ao in range(end_dp - 1 if end_dp == dp else end_dp):
...
class BinomialUtils:
    ...
    """
    exact_log_factorial = numpy.cumsum([numpy.log(i) if i > 0 else 0.0 for i in range(256 + 1)])
    ...
    def log_binomial_cdf(x, n, log_p, log_q=None):
        ...
        if log_q is None:
            log_q = BinomialUtils.log_complement_from_log_p(log_p)
        log_pmf = [BinomialUtils.log_binomial_pmf(n, k, log_p, log_q) for k in range(int(x) + 1)]
...
class LodManager:
    ...
    def set_parameters(self, params):
        self.__min_var_coverage = params.get('min_var_coverage', self.__min_var_coverage)
        assert (self.__min_var_coverage >= 0)
        self.__min_variant_score = params.get('min_variant_score', self.__min_variant_score)
        ...
        self.__min_callable_prob = params.get('min_callable_prob', self.__min_callable_prob)
        assert (0.0 < self.__min_callable_prob < 1.0)
        self.__min_allele_freq = params.get('min_allele_freq', self.__min_allele_freq)
    ...
    def __callable_prob(self, dp, af, min_callable_ao, qual_plus=None, qual_minus=None):
        ...
        log_p = numpy.log(af)
        log_q = BinomialUtils.log_complement_from_log_p(log_p)
        log_pmf_list = [BinomialUtils.log_binomial_pmf(dp, ao, log_p, log_q) for ao in range(min_callable_ao)]
        if not log_pmf_list:
            return 1.0
    ...
    # @staticmethod
    # def __callable_prob_and_its_derivative(dp, af, min_callable_ao, qual_plus=None, qual_minus=None):
    #     assert (0.0 <= af <= 1.0)
    #     log_p = numpy.log(af)
    #     log_q = BinomialUtils.log_complement_from_log_p(log_p)
    #
    ...
    #     ro_ary = dp - ao_ary
    #     log_n_choose_k_ary = numpy.array([BinomialUtils.log_n_choose_k(dp, ao) for ao in range(min_callable_ao)], dtype=float)
    #
    ...
    def __calculate_lod_by_line_search(self, dp):
        ...
        new_start_af = new_end_af = p_callable_new_start = p_callable_new_end = None
        ...
        for round_idx in range(max_rounds):
            ...
            for af in af_ary:
                ...
                if new_start_af is not None and new_end_af is not None:
                    start_af = new_start_af
                    end_af = new_end_af
                    break

            # Stopping if we are very close
            if p_callable_new_start is not None and p_callable_new_end is not None:
                if abs(p_callable_new_start - p_callable_new_end) < 0.001 * self.__min_callable_prob:
                    return linear_interpolation(
                        self.__min_callable_prob, p_callable_new_start, new_start_af, p_callable_new_end, new_end_af
                    )
...
if __name__ == '__main__':
    ...
    parser.add_option('-p', '--parameter-file', help='Read the TVC hotspot parameters in the files for getting -v, -s, -f ',
                      dest='param_path')
    parser.add_option('-v', '--min-var-coverage',
                      help=f'NOCALL if variant allele molecular coverage below this [{PARAM_DICT["min_var_coverage"]}]',
                      dest='min_var_coverage')
    parser.add_option('-s', '--min-variant-score',
                      help=f'NOCALL if variant allele molecular coverage below this [{PARAM_DICT["min_variant_score"]}]',
                      dest='min_variant_score')
    parser.add_option('-f', '--min-allele-freq',
                      help=f'Allele frequency cut-off for a variant call [{PARAM_DICT["min_allele_freq"]}]', dest='min_allele_freq')
    parser.add_option('-c', '--min-callable-prob',
                      help=f'Minimum callable probability of the variant [{PARAM_DICT["min_callable_prob"]}]',
                      dest='min_callable_prob')
    ...
    parser.add_option('-a', '--axis-type',
                      help='(Optional) X-Y axis for plotting LOD vs MDP. Options: {linear, loglog, semilogx, semilogy} [linear]',
                      dest='axis_type')
    ...
    if options.mdp is None:
        ...
        dp_list = map(int, getattr(numpy, options.mdp))
    else:
    ...
    if options.fig_path is not None:
        ...
        title_text = ', '.join('%s=%s' % (k, str(v)) for k, v in PARAM_DICT.items()) + '\n'
